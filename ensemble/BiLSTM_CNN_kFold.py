...
for fold_id, model in enumerate(models):
    model_path = os.path.join(conf.model_path, "BiLSTM_CNN_new_{0}_weights.npy".format(fold_id))
    np.save(model_path, model.get_weights())

    test_predicts_path = os.path.join(conf.output_path, "BiLSTM_CNN_new_predicts{0}.npy".format(fold_id))
...
sample_submission = pd.read_csv("../input/sample_submission.csv")
sample_submission[list_classes] = test_predicts
sample_submission.to_csv("../output/trained_models/sub2.csv", index=False)

sample_submission = pd.read_csv("../input/train.csv")
sample_submission[list_classes] = trained_y
sample_submission.to_csv("../output/trained_models/oof2.csv", index=False)